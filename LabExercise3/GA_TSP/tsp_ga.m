...
function varargout = tsp_ga(varargin)
    ...
    for iter = 1:numIter
        ...
        parent1 = pop(p1ind, :);
        parent2 = pop(p2ind, :);
        child1 = zeros(1, length(parent1));
        whereToStart = randi(n-1);
        nbrToExtract = randi(n-whereToStart+1);
        index = 0;
        for i=1:nbrToExtract
            child1(1,whereToStart + index) = parent1(1,whereToStart + index);
            index = index + 1;
        end
        
        toAddFromParent2 = setdiff(parent2, child1, 'stable');
        j = 1;
        for i=1:length(child1)
            if (child1(1,i) == 0)
                child1(1,i) = toAddFromParent2(1,j);
                j = j+1;
            end
        end
